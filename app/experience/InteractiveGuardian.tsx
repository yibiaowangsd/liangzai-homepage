...
export default function InteractiveGuardian() {
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      try{
        ...
        const instance=createHeroScene(canvas.current,fallback,controller.signal,next=>{if(alive)setPhase(next);},next=>{if(alive)setFusion(next);});
  ...
  const instruction=fusion==="merging"?"量子与勇气，正在成为同一种力量":fusion==="fused"?"靓龙 · 拖动探索全新形态，点击胸核唤起共鸣":fusion==="charging"?"继续长按，开启双星融合":fallback?"3D 暂不可用，可切换图片视角":status==="loading"?"星云正在苏醒":formed?(mode==="duo"?"继续长按，融合为靓龙 · 拖动或点击仍可互动":"拖动旋转 · 点击角色，发现回应"):phase==="revealing"?"星光凝实，伙伴降临":phase==="gathering"?"继续长按，让星光凝聚成形":"划过星云，拨动星尘 · 长按唤醒"+labels[mode];
  return (
    <dialog ref={shell} open className="guardian-stage" aria-label={immersive?"靓龙融合空间":"角色星云展台"} onCancel={event=>{event.preventDefault();runtime.current?.exitFusion();}} data-fusion={fusion} data-status={status} data-model={mode} data-arrival={phase} aria-busy={status==="loading"}>
      {immersive&&<><button className="guardian-fusion-back" onClick={()=>runtime.current?.exitFusion()}>返回双人展示</button><div className="guardian-fusion-heading"><p>量仔 × 奶龙</p><h2>{fusion==="fused"?"靓龙":"双星融合"}</h2><span>{fusion==="fused"?"量子之力，勇气之心。":"两份力量，一个全新的守护者。"}</span></div></>}
      ...
      <canvas ref={canvas} className="guardian-canvas" tabIndex={status==="ready"?0:-1} role="img"
        aria-label={fusion==="fused"?"靓龙合体模型。拖动或方向键旋转，点击模型或回车互动，Escape 返回双人展示。":fusion==="merging"?"正在融合为靓龙，Escape 返回双人展示。":formed?`${labels[mode]}三维模型。拖动旋转，点击身体部位互动，方向键旋转；双人模式按住空格或回车开始融合，其余模式回车唤起星光。`:`${clouds[mode]}。长按鼠标或按住空格、回车，粒子将凝聚为${labels[mode]}。松开可让未成形的星光散开。`}
      ...
      <div className="guardian-scene-label"><span className="guardian-live-dot" />{fusion==="fused"?"靓龙":formed?labels[mode]:clouds[mode]}<span>{formed?"已唤醒":"等待共鸣"}</span></div>
