...
原项目使用的 PQMagic 上游提交记录和许可证副本分别为 `public/pqc-practice/PQMagic-UPSTREAM_COMMIT.txt` 与 `public/pqc-practice/PQMagic-LICENSE.txt`。工作台沿用量仔主页的导航、黑银与冰蓝风格，主页设有专门的实战入口；更新 PQMagic 模块时，应同步核对 JS 与 WASM 的配套关系，并保留征集目录功能。
