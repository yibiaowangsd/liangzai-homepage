...
页底提供 [ngcc.dev](https://ngcc.dev/reports/index.html) 已公开发现的中文摘要，当前收录 Aigis-Enc+、Aigis-Sig+ 和 WeaverKEM；显示报告所描述的**归档提交源码**状态，改动过的浏览器构建与原报告不是完全相同的二进制。所有候选都可从页面打开报告索引；未整理中文摘要并不表示没有问题。征集候选与现有 PQMagic 算法即使名称相近也不能视为相同实现。
