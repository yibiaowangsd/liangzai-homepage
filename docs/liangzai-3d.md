...
- **局部扰动**：删除 `world.xy += uPointer.xy ...` 全场平移。排斥、涡流、点击波纹及近鼠标增亮均有 1.25 世界单位的硬边界，边界外权重为零；保留六点平滑轨迹。
- **角色差异**：量仔改为三组不同倾角的椭圆量子轨道，中心留空、星尘细密；奶龙改为非对称的弯曲云柱和膨松团块，使用更厚的柔雾和局部翻涌。不是同一几何仅更换颜色。两者共用稀疏远星，双人分别保留各自星云。
- **天体细节**：使用预生成的 256×128 无接缝球面贴图，为恒星添加颗粒、气态行星添加条带/风暴、蓝色行星添加海陆/独立云层/边缘大气、天然卫星添加陨坑和凹凸。分层冰尘环含缝隙，通信卫星添加框架/抛物面轮廓接收盘及实例化太阳能电池格，彗星加入实体核与连续弯曲尘埃尾。纹理只初始化一次，不逐帧生成；增加几何细节但维持预算。
- **成形保留**：每种展示模式持有独立出场状态，切换后不再无条件 `arrival.reset()`；已成形角色在本次页面会话中保持。单人、双人和融合都有约 3 秒的低亮度粒子余辉，之后模型仍保留，融合只在显式返回时退出。材质预编译期间暂停提交画面，防止缓存状态下露出未准备好的模型。
- **蓝光检查**：确定存在每 5.2 秒缩放 LED 眼睛的自动眨眼，以及天线点击把 `pose.energy` 拉至 .85、联动共享蓝色环材质的链路。取消自动眨眼和天线的全环闪光，保留明确点击眼睛后的动作。曝光固定为 .96，轮廓灯改为低强度淡中性色，出场 Bloom 峰值降低，完成后亮度包络精确归零。未在真实浏览器复现用户所见闪光，因此这两条是代码可确认的亮度变化来源，不能认定是唯一原因。
- **合体造型**：本次不改 `fusion-model.ts`；新造型先提供独立的正/侧/背三视图供选择，用户确认后再建模。

回归验证覆盖模式切换保留、融合完成不回退、3 秒余辉衰减、远处粒子扰动权重为零、同种随机输入下两种星云的几何差异、纹理接缝及天体几何预算、LED 眼睛待机稳定。当前环境仍未完成真实 WebGL 设备的视觉和帧率验收。


### A2 量子奶龙仔与返回后保留

用户选定 A2 三视图后，融合模型改为「量子奶龙仔」。屏幕脸、耳机、弹簧天线直接复用原量仔头部几何，克隆独立材质和关节状态，保留原始源文件；耳部换为小型圆角金色角及白色根座。身体重建为连续梨形旋转曲面、贴合曲面的浅色肚皮、短腿圆脚趾、白肩片和逐渐收细的厚尾。胸部为内嵌 Q 核心。模型高约 5.29，宽约 2.90，约 23.4 万三角形，头部几何在同一场景内共享，不重复下载资源。仅双人模式创建，退出场景统一释放共享资源。

恢复量仔约 5.2 秒一次的自动眨眼，新合体模型也有独立自动眨眼；关闭动效时睁眼静止。眨眼只变换眼部，不改变曝光、场景灯光或材质发光强度。保留之前取消天线全环闪光的处理。

融合新增 `docked` 状态：完成后点击“返回首页展台”或 Escape 只关闭全屏，继续在首页显示合体形态。切换到量仔/奶龙单人后，再选“合体形态”仍显示已完成模型。只有融合未完成时提前返回才取消本次融合。状态保存在当前页面会话内，不跨刷新保存。修正完成后的蓄力值仍为 1、误将普通点击当作长按而吞掉点击互动的问题。

验证覆盖真实头部复用和材质隔离、合体模型尺寸与有限几何、粒子表面采样、尾部轮廓、自动眨眼及暂停、融合结果返回与切换保留、完成后点击可用；构建及相关测试通过。另作离线三方向网格检查；该检查不代表浏览器 PBR、阴影或实际帧率验收。
